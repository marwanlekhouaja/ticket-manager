...
<div class="d-flex mt-3">
    ...
    <div class="w-50 bg-light p-4 d-flex align-items-center justify-content-center flex-column">
        <h2 style="font-family: monospace">{{__("messages.Espace client")}}</h2>
        <p style="font-family: monospace; font-size:18px;">
            {{__("messages.Vous souhaitez suivre votre consommation, gérer vos factures, ou obtenir de l'aide ? Pour accéder à ces fonctionnalités, il vous suffit de demander un ticket")}}
            <form id="ticketForm" action="{{ route('ticket.create') }}" method="POST">
                ...
                <button type="submit" class="btn btn-secondary text-decoration-none text-light">{{__("messages.ici")}}</button>  
            </form>         
        </p>
    </div>
</div>
